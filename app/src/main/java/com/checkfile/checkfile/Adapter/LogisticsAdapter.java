...
import com.checkfile.checkfile.View.CustomTextView;
...
public class LogisticsAdapter extends BaseAdapter {
	...
	public LogisticsAdapter(List<LogisticsInfo> list, Context context) {
		...
		for (int i = 0; i < list.size() + 2; i++) {
			// 倒数第二个用于装载item顶部内边�?,倒数第一个用于装载listview分割线高�?
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		ImageView  yesOrNo=(ImageView) convertView.findViewById(R.id.yesorno);
		yesOrNo.setBackgroundResource(list.get(position).getBrc());
